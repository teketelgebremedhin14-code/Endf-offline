    ...
    useEffect(() => {
        ...
        if (translationActive) {
            ...
            interval = setInterval(() => {
                fetchChatter();
            }, 8000); // Fetch new batch every 8 seconds
        }
        return () => clearInterval(interval);
    }, [translationActive]);
    ...
    const fetchChatter = async () => {
        ...
        if (batch && batch.length > 0) {
            batch.forEach((item: any, i: number) => {
                setTimeout(() => {
                    setLiveTranscript(prev => [`[${new Date().toLocaleTimeString()}] ${item.org} >> ${item.trans}`, ...prev].slice(0, 15));
                }, i * 1500); // Stagger updates
            });
        }
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 flex flex-col h-[calc(100vh-140px)]">
            <div className="flex justify-between items-center mb-2 flex-shrink-0">
                <div>
                    <h2 className="text-2xl font-bold text-white tracking-tight">{t('comms_title')}</h2>
                    <p className="text-gray-400 text-sm">{t('comms_subtitle')}</p>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-military-800 rounded-lg border border-military-700 flex flex-col">
                        ...
                        <div className="p-4 flex-1 h-48 overflow-hidden relative">
                            {translationActive ? (
                                <div className="space-y-2 h-full overflow-y-auto font-mono text-xs">
                                    {liveTranscript.map((line, i) => (
                                        <div key={i} className="text-gray-300 border-l-2 border-purple-500 pl-2 animate-in slide-in-from-left-2">
                                            {line}
                                        </div>
                                    ))}
                                    {liveTranscript.length === 0 && <div className="text-gray-500 italic">Listening for multi-lingual chatter...</div>}
                                </div>
                            ) : (
