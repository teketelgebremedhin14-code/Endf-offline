...
import { Users, UserPlus, GraduationCap, FileBadge, Navigation, MapPin, BookOpen, Clock, ShieldCheck, Heart, User, ChevronDown, ChevronRight, BrainCircuit, TrendingUp, Target, Search, RefreshCw, AlertOctagon, TrendingDown, UserX, Activity } from 'lucide-react';
...
import { generateDynamicData, analyzePersonnelRisk } from '../services/localAiService';
      ...
      <div className="flex justify-between items-center mb-2 flex-shrink-0">
        ...
        <div className="flex space-x-2 bg-military-800 p-1 rounded-lg border border-military-700">
            ...
            <button onClick={() => setActiveTab('risk')} className={`px-3 py-1.5 text-xs font-bold rounded flex items-center ${activeTab === 'risk' ? 'bg-red-600 text-white' : 'bg-military-800 text-gray-400'}`}>
                <AlertOctagon size={12} className="mr-1" /> RISK & RETENTION
            </button>
            <button onClick={() => setActiveTab('org')} className={`px-3 py-1.5 text-xs font-bold rounded ${activeTab === 'org' ? 'bg-blue-600 text-white' : 'bg-military-800 text-gray-400'}`}>CHAIN OF COMMAND</button>
